...
\vspace*{-1cm}
\includegraphics[scale=0.3]{v_histogram.pdf}\\
\vspace*{-1cm}
...
\column{0.5\textwidth}
{\small
ifølge Maxwell Boltzmann, og vi ser at det er veldig tett med partikler der. Det ytterste skallet viser partikler med bevegelsesmengde $p_2$. Her er det ikke så mange partikler siden vi er langt vekk fra origo. Men innser du at det kanskje kan være minst like mange og kanskje {\bf flere} partikler innenfor dette skallet? Rett og slett fordi området som skallet omfatter er større og får da med flere partikler selv om det er mindre tetthet av partikler her?\\
...
\hyperlink{prom4c}{\pagebutton{Neste side}}
\end{columns}
\end{frame}
...
\item \textcolor{yellow}{Antalltettheten av partikler med bevegelsesmengde $\vec{p}$ er $n(\vec{p})=n(p_x)n(p_y)n(p_z)$. \textcolor{red}{Men for å få et faktisk antall per volum må vi igjen gange opp med intervallet  $dp_xdp_ydp_z$: $n(\vec{p})dp_xdp_ydp_z=n(p_x)n(p_y)n(p_z)dp_xdp_ydp_z$}.}
\end{itemize}
...
\hyperlink{prom4c2}{\pagebutton{Neste side}}
\end{frame}
}
...
\hyperlink{prom4d}{\pagebutton{Neste side}}
\end{frame}
}
...
\begin{frame}
\label{prom4d}
\lastpagebutton{prom4c2}
\textcolor{yellow}{
{\bf Hva hvis vi vil ha antalltettheten av partikler med absoluttverdi mellom $p$ of $p+dp$ av bevegelsesmengden?} Altså alle partikler med absoluttverdi $p$ av bevegelsesmengden. \textcolor{red}{Disse befinner seg vel på et skall i avstand $p$ fra origo i $\vec{p}$-rommet med tykkelse $dp$?} Volumet $V_p$ av et slikt kuleskall er vel $4\pi p^2dp$? Hvis vi {\bf ganger opp antalltettheten $n(\vec{p})$ med dette volumet $V_p$, så får vi antall partikler per fyisk volum $V$} med absoluttverdi av bevegelsesmengde mellom $p$ og $p+dp$? Altså $n(\vec{p})4\pi p^2 dp$.} \\
\vspace*{0.5cm}
\textcolor{yellow}{
...
\hyperlink{prom4e}{\pagebutton{Neste side}}
\end{frame}
}
...
\hyperlink{prom4f}{\pagebutton{Neste side}}
\end{frame}
}
